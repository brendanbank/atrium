...
| 0.22.0 | unchanged | — (mobile UX polish: the entire HomePage welcome card (greeting + intro + Profile/Notifications/Admin buttons) now drops as a unit when a host registers a home widget instead of leaving orphan action buttons (closes #100); sidebar `NavLink` parents are controlled so the second tap on an already-open group reliably collapses on iOS WebKit (closes #102); `<AppShell.Navbar>` wraps its content in a `ScrollArea` so all admin/settings tabs stay reachable on phone-sized viewports where the dynamic browser toolbar would otherwise hide the bottom items (closes #103); `overscroll-behavior-y: none` on `html, body` disables iOS rubber-band at the document edge that interacted with the collapsing toolbar to hide content (closes #104); admin tables (`users`, `roles`, `audit`, `reminders`, `email-outbox`, `email-templates`, `translations`) switch to a vertical stack of row-cards below 48em instead of a horizontally-scrolling `Table.ScrollContainer` (closes #107); `Burger` carries `aria-label={t('nav.toggle')}`) | — | — |
